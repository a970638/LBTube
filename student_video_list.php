            ...
            <?php
              ...
              foreach ($video_list as $video_list) {
                ...
                echo '<td>
                  ...
                  // }
                //查詢影片評論  
                $sql1 = "SELECT  ROUND(AVG(q1),1), ROUND(AVG(q2),1), ROUND(AVG(q3),1), ROUND(AVG(q4),1), ROUND(AVG(q5),1) FROM comment WHERE video_id = '$video_list' AND `type` = 'cm' AND comment_situation NoT IN ('old')";
                $result1 = mysqli_query($conn, $sql1);
                if(mysqli_num_rows($result1) > 0){
                  $row = mysqli_fetch_assoc($result1);
                  if(empty($row['ROUND(AVG(q1),1)']) == false){
                    echo '<td style="font-size:13px;">平均分數（5為最高，1為最低）
                          <br>影片中解題過程及解答是否正確：'.$row['ROUND(AVG(q1),1)']
                        .'<br>影片中解釋的觀念是否清楚易懂：'.$row['ROUND(AVG(q2),1)']
                        .'<br>影片整體講解過程是否順暢：'.$row['ROUND(AVG(q3),1)']
                        .'<br>影片設計、講解方式是否吸引你觀看：'.$row['ROUND(AVG(q4),1)']
                        .'<br>影片的畫面及聲音是否清晰：'.$row['ROUND(AVG(q5),1)'].'</td>';
                  }else{
                    echo '<td>尚未有評論</td>';
                  }  
                }
                ...
                echo '<button type="button" id="delete-button" class="btn btn-danger" style="display:block; margin-top: 5px;" 
                      onclick="if(confirm(\'刪除影片將會一併刪除對此影片的所有評論及留言，確定要繼續？\'))
                      {document.getElementById(\'loadingOverlay\').style.display = \'flex\';location.href=\'delete.php?video_name='.$video_list.'\';}">
                      刪除影片</button></td>';
                echo '</tr>';
                $i++;
              }
